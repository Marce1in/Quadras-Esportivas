...
function Login({atlas}: {atlas: Atlas}){
    ...
    function logar(data: any){
        atlas.contas.logar(data.nome, data.senha)        

        if(atlas.contas.sessao == ""){
            alert("Senha ou Nome inválidos!")
            return
        }

        if(atlas.db.membros[atlas.contas.sessao].admin){
            navegador("/admin")
        }
        else navegador("//")
    }
        ...
        <>
            <main className='flex h-screen justify-center items-center'>
                ...
                >
                    ...
                    <button type="submit" className="btn btn-primary w-32">Logar</button>

                    <span className='text-base-content pt-5'>Não tem uma conta?</span>
                    <Link to="/registro" className='btn w-32'>Registre-se</Link>
                </form> 
            </main>
        </>
    )
